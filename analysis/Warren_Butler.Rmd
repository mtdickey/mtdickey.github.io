...
## Bayesian Approach
p <- seq(from=0.055, to=0.12, by=0.005) ## EFG for these players ranges from 0.06 to 0.12.  Assume uniform prior.
prior <- rep(1/length(p), length(p))
war_likelihood <- dbinom(warren_succ, size = warren_n, prob = p)
but_likelihood <- dbinom(butler_succ, size = butler_n, prob = p)
...
### Put Posteriors in a data.frame to visualize in ggplot
tov_posterior_df <- data.frame(Player = c(rep("Warren", 14), rep("Butler", 14)),
                               EFG = c(seq(from=0.055, to=0.12, by=0.005), seq(from=0.055, to=0.12, by=0.005)),
                               probs = c(war_posterior, but_posterior))
...
## Bayesian Approach
p <- seq(from=0.07, to=0.50, by=0.001) ## EFG for these players ranges from 0.06 to 0.12.  Assume uniform prior.
prior <- rep(1/length(p), length(p))
war_likelihood <- dbinom(warren_succ, size = warren_n, prob = p)
but_likelihood <- dbinom(butler_succ, size = butler_n, prob = p)
